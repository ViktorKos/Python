...
@ input_error
def show_data_handler(user_data: tuple, address_book: Address_Book):
    # letters or numbers
    request_keyword = user_data[0] if user_data[0] != "" else user_data[1]
    book = address_book.find_by_keyword(request_keyword)
    if book is not None:
        responseMessage = show_all_phones_handler(None, book)
    else:
